        ...
        <div v-for="(item, ind) in items" :key="item.id">
            <div class="d-flex align-items-center justify-content-between py-1">
                <div class="mr-3">
                    ...
                    <p class="text-dark9 font-09 text-nowrap my-2">{{$t('spec')}}：{{item.property}} ￥{{item.price1}}/{{gsv(['name', $i18n.locale], item.unit)}}</p>
...
export default {
    ...
    methods: {
        getList() {
            this.isLoading = true
            myExpoOffice(8, this.my_expo_office_id).then(res => {
